...
recv:
	while(net->state != NET_STATE_SHUTDOWN) {
		...
		// printf("Want to read %lu bytes\n", ring_free_space_contig(ring));

		// read_len = read(socket, ring->ptr_write, ring_free_space_contig(ring));
		for(int seekCounter = 50; seekCounter > 0; seekCounter--) { 
			ring->ptr_write = ring->data;
			ring->ptr_read = ring->data;
			read_len = recv(socket, ring->ptr_write, ring_free_space_contig(ring), MSG_WAITALL);
		...
		while(ring_any_available(ring)) {
			...
			if(!ring_memcmp(ring, "USE MORE BANDWITH! PX ", strlen("USE MORE BANDWITH! PX "), NULL)) {
...
//						printf("Got pixel outside screen area: %u, %u outside %u, %u\n", x, y, fbsize->width, fbsize->height);
			...
			} else if(!ring_memcmp(ring, "PLEASE USE MOAR BANDWITH, PLEASE USE MOAR BANDWITH, PLEASE USE MOAR BANDWITH, PLEASE USE MOAR BANDWITH, PLEASE USE MOAR BANDWITH: SIZE",
				strlen("PLEASE USE MOAR BANDWITH, PLEASE USE MOAR BANDWITH, PLEASE USE MOAR BANDWITH, PLEASE USE MOAR BANDWITH, PLEASE USE MOAR BANDWITH: SIZE"), NULL)) {
